...
#     include "CMN_SIZE"       ! Size parameters
      ...
      REAL*8              :: CURR_GC_AOD(IIPAR,JJPAR,MAX_MODIS_TRC)
      REAL*8              :: CURR_MODIS_AOD(IIPAR,JJPAR)
      REAL*8              :: CURR_COST(IIPAR,JJPAR)

      REAL*8              :: ALL_GC_AOD(IIPAR,JJPAR,MAX_MODIS_TRC) = 0D0
      REAL*8              :: ALL_MODIS_AOD(IIPAR,JJPAR)            = 0D0
...
!      INTEGER                :: START2(3), COUNT2(3)
  ...
  90  FORMAT(' - READ_MODIS_AOD_OBS: No data file (warning) in', I10)
...
#     include      "CMN_SIZE"      ! Size params
      ...
      REAL*8                      :: TMP_COST ! (ywang, 04/07/2017)
...
!         ENDDO
         ! (ywang, 04/14/2017)
         IF (N_MODIS_OBS > 0) THEN
            MODIS_DAYFRC(1:N_MODIS_OBS) = MODIS(1:N_MODIS_OBS)%TIME(1)
         END IF
      ...
      IF ( N_MODIS_OBS == 0 ) THEN
         ...
         PRINT*, '   - CALC_MODIS_AOD_FORCE: No MODIS AOD observations 
     &for current day'

         IF (  ABS( GET_TAUb() - GET_TAU() ) < 1E-6 ) THEN

            CALL MAKE_AVERAGE_MODIS_AOD
     ...
     &                   * 1d9 / AD(I,J,L)
               ...
               TMP_COST       = .5d0*DIFF(L,T)*FORCE(L,T)
...
!------------------------------------------------------------------------------
!
      SUBROUTINE MAKE_AVERAGE_MODIS_AOD
!
!******************************************************************************
! Subroutine MAKE_AVERAGE_MODIS_AOD output some diagnostic data for
! simulation time
! (ywang, 04/07/2017)
!
!******************************************************************************
!
      ...
      !============================================================
      ! MAKE_AVERAGE_MODIS_AOD begins here!
      !============================================================

      ! Hardwire output file for now
      OUTPUT_FILE = 'gctm.modis.aod.ave.YYYYMMDD.NN'
 ...
 100  FORMAT( '     - MAKE_AVERAGE_MODIS_AOD:  Writing ', a )
      ...
      CALL CHECK( NF90_DEF_DIM( FID, "lev", LLPAR, LEV_DIM_ID ), 102 )

      !----------------------------------------
      ! Define variables and assgin attrubute
      !----------------------------------------
      CALL CHECK( NF90_DEF_VAR( FID, "cf", NF90_FLOAT, 
     &            (/LON_DIM_ID,LAT_DIM_ID/), CF_ID), 200 )
      CALL CHECK( NF90_PUT_ATT( FID, CF_ID,
     &            "longname", "cost function"), 300 )
      CALL CHECK( NF90_PUT_ATT( FID, CF_ID,
     &            "unit", "unitless"), 400)
      ...
      !---------------------------
      ! provide vaules for variable
      !---------------------------
      CALL CHECK( NF90_PUT_VAR( FID, CF_ID, REAL(ALL_COST, 4)), 500 )
       
      
      ! Close the file
      CALL CHECK( NF90_CLOSE( FID ), 9999 )

      END SUBROUTINE MAKE_AVERAGE_MODIS_AOD
!
!------------------------------------------------------------------------------
!
