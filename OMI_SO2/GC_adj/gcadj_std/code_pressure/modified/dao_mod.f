...
!  (63) AD       (REAL*8 ) : Total air mass                      [kg]
...
!  (31) Add dry mole ratio and wet mass ratio unit conversions (mml, 4/15/15)
!******************************************************************************
!
      IMPLICIT NONE
...
!----------------------------------------------------------
      ...
      SUBROUTINE SET_DRY_SURFACE_PRESSURE(PS)
!
! removes the water vapor pressure and resets PS1 and PSC2
! uses DELP array as a temporary storage
!
...
! local variables
      ...
      !Variable PTOP is required n order to achieve the true reversability 
      !between the dry surface pressure and the wet surface pressure.
      !But, the true reversability is not required.
      !real*8    :: VAR_PTOP(IIPAR, JJPAR)
      !GET_VAR_PTOP(PS, VAR_PTOP)
      ...
      DO I = 1, IIPAR
        ...
        !pDry = VAR_PTOP(I,J)
        DO L = 1, LLPAR
          P1 = GET_AP(L) + GET_BP(L) * PS(I,J) 
          P2 = GET_AP(L+1) + GET_BP(L+1) * PS(I,J)
          pDry = pDry + (P1-P2) *(1-SPHU(I,J,L)*1.0e-3)
        ENDDO
        if (pDry < 0) then
          print *, 'invalid PS: ', I, J, pDry, PS(I,J)
          print *, 'SPHU: ', SPHU(I,J,:)
          STOP
        endif
        PS(I,J) = pDry
...
!-----------------------------------------------------------------------------------
      SUBROUTINE CHECK_REVERSABILITY(PS)
!
! removes the water vapor pressure and resets PS1 and PSC2
! uses DELP array as a temporary storage
!
...
! local variables
      real*8    :: P1, P2, pDry, pWet
      real*8    :: VAR_PTOP(IIPAR, JJPAR), TMP, VAR
      integer   :: I, J, L

! must generate DELP & AD to test GET_WET_SURFACE_PRESSURE here
      DO J = 1, JJPAR
      DO I = 1, IIPAR
        DO L = 1, LLPAR
          P1 = GET_AP(L) + GET_BP(L) * PS(I,J) 
          P2 = GET_AP(L+1) + GET_BP(L+1) * PS(I,J)
          DELP(L,I,J) = (P1-P2)*(1-SPHU(I,J,L)*1.0e-3)
          AD(I,J,L) = DELP(L,I,J) * G0_100 * GET_AREA_M2(J)
        ENDDO
      ENDDO
      ENDDO

      CALL GET_VAR_PTOP(PS, VAR_PTOP)
      ...
      DO I = 1, IIPAR
        pDry = VAR_PTOP(I,J) 
        DO L = 1, LLPAR
          pDry = pDry + DELP(L,I,J) *(1-SPHU(I,J,L)*1.0e-3)
        ENDDO
        if (pDry < 0) then
          print *, 'invalid PS: ', I, J, pDry, PS(I,J)
          print *, 'SPHU: ', SPHU(I,J,:)
        endif
        pWet = GET_WET_SURFACE_PRESSURE(I,J)
        TMP = pWet - PS(I,J)
        VAR = VAR+TMP*TMP
        print *, J,',',I,',',TMP
      ENDDO
      ...
      print *, 'variance =', SQRT(VAR/(IIPAR*JJPAR))

      END SUBROUTINE CHECK_REVERSABILITY 

!-----------------------------------------------------------------------------------
      SUBROUTINE GET_VAR_PTOP(wetPS, VAR_PTOP)

      USE PRESSURE_MOD, ONLY : GET_AP, GET_BP

! arguments
      real*8, intent(in)    :: wetPS(:,:)
      real*8, intent(inout) :: VAR_PTOP(:,:)

      integer   :: L, I, J
      real*8    :: pDry, pWet, P1, P2
      ...
      DO J = 1, JJPAR
        DO I = 1, IIPAR
          ...
          pWet = 0
          DO L = 1, LLPAR
            P1 = GET_AP(L) + GET_BP(L) * pDry
            P2 = GET_AP(L+1) + GET_BP(L+1) * pDry
            pWet = pWet +(P1-P2)/(1-SPHU(I,J,L)*1.0e-3)
          ENDDO
          VAR_PTOP(I, J) = wetPS(I,J) -pWet
        ENDDO
      ENDDO

      END SUBROUTINE GET_VAR_PTOP

!------------------------------------------------------------------------------------
      SUBROUTINE CHECK_MASS_BALANCE(dryPS)

      USE PRESSURE_MOD, ONLY : GET_AP, GET_BP

! arguments
      real*8, intent(in) :: dryPS(:,:)

      integer   :: L, I2, J2, COUNT
      real*8    :: TMP1, TMP2, TMP3, VAR, VAR_RATE
      real*8    :: P1, P2, delP1, delP2, delPVar(47)
      ...
      DO J2 = 1, JJPAR, 10
        DO I2 = 1, IIPAR, 10
          ...
          TMP2 = 0.02
          DO L = 1, LLPAR
            P1 = GET_AP(L) + GET_BP(L) * dryPS(I2,J2)
            P2 = GET_AP(L+1) + GET_BP(L+1) * dryPS(I2,J2)
            TMP2 = TMP2+(P1-P2)/(1-SPHU(I2,J2,L)*1.0e-3)
          ENDDO
      
          ! find the discrepancy between the pseudo airmass derived from
          ! wetair and dryair
          DO L = 1, 47
            P1 = GET_AP(L) + GET_BP(L) * TMP2
            P2 = GET_AP(L+1) + GET_BP(L+1) * TMP2
            delP1 = P1-P2

            P1 = GET_AP(L) + GET_BP(L) * TMP1
            P2 = GET_AP(L+1) + GET_BP(L+1) * TMP1
            delP2 = (P1-P2) + delP1*(SPHU(I2,J2,L)*1.0e-3)

            delP2 = delP2-delP1
            delP2 = delP2*delP2
            delPVar(L) = delpVar(L) + delP2
          ENDDO
          COUNT = COUNT+1
        ENDDO
      ENDDO
      DO L = 1, 47
        print *, L,',',SQRT(delPVar(L)/COUNT)
      ENDDO
...
! local variables
      ...
      DO L = 1, LLPAR
        profile(L) = GET_AP(L) + GET_BP(L)*wetPS
      ENDDO
...
!$OMP+PRIVATE( I, J, L )
      ...
      DO I = 1, IIPAR
         ...
         ! 3D Variables: at midpt of dyn timestep
         SPHU(I,J,L) = SPHU1(I,J,L) + (SPHU2(I,J,L) - SPHU1(I,J,L)) * TM
         if (SPHU(I,J,L) >= 1.0e3) then
           print *, 'invalid SPHU:', I,J,L, SPHU1(I,J,L), SPHU2(I,J,L)
           STOP
         endif
         T(I,J,L)    = TMPU1(I,J,L) + (TMPU2(I,J,L) - TMPU1(I,J,L)) * TM
      ENDDO
...
!                          =3,4 removed
!                          =5 then convert from [kg] --> [kg/kg]
!                          =6 then convert from [[kg/kg] --> [kg]
!  (2 ) NTRACE (INTEGER) :
!  (3 ) TCVV   (REAL*8 ) : Array containing [Air MW / Tracer MW] for tracers
!
!  Arguments as Input/Output:
!  ======================================================================
!  (4 ) STT    (REAL*8 ) : Array containing tracer concenttation
!
!  NOTES:
...
!  (8 ) Added dry mol. mixing ratio conversion (mml, 6/10/15)
!  (9 ) Added wet mass mixing ratio conversion (mml, 6/10/15)
!******************************************************************************
!
      ! References to F90 modules
...
!$OMP END PARALLEL DO
         ...
         CASE ( 3 ) 
            CALL ERROR_STOP( '3 is Invalid IFLAG value !',
     &                       'CONVERT_UNITS (dao_mod.f)' )

         CASE ( 4 ) 
            CALL ERROR_STOP( '4 is Invalid IFLAG value !',
     &                       'CONVERT_UNITS (dao_mod.f)' )

         !==============================================================
         ! IFLAG = 5: Convert from [kg] -> [kg/kg]
         !
         !  STT[kg/kg] = STT [kg]/ total air mass
         !==============================================================
         CASE ( 5 )
!$OMP PARALLEL DO
!$OMP+DEFAULT( SHARED )
...
!$OMP END PARALLEL DO
         ...
         !==============================================================
         ! IFLAG = 6: Convert from [kg/kg] -> [kg]
         !
         ! STT[kg] = STT [kg/kg] * total air mass
         !==============================================================
         CASE ( 6 )
...
!$OMP END PARALLEL DO
         ...
         !==============================================================
         CASE DEFAULT
            CALL ERROR_STOP( 'Invalid IFLAG value (must be 1-6)!',
     &                       'CONVERT_UNITS (dao_mod.f)' )
      END SELECT
