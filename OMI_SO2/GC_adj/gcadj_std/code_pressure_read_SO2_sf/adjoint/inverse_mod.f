...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE YW_READ_SF_FILE ( )
!
!******************************************************************************
!  Subroutine READ_SF_FILE reads the gctm.sf.* file into ICS_SF or
!  EMS_SF
!  (dkh, 9/17/04)
!  Subroutine YW_READ_SF_FILE read the gctm.sf file into EMS_SF
!
!  Module Variable as Input:
!  ============================================================================
!  (1 ) N_CALC    : Current iteration number
...
!  (1 ) Add support for ACTIVE_VARS == 'EMISSIONS' case (dkh, 11/27/04)
!  (2 ) Add support for ACTIVE_VARS == 'FDTEST' case (dkh, 02/17/05)
!  (3 ) Now use CATEGORY = 'IJ-EMS-$' for ACTIVE_VARS == 'EMISSIONS'
!  case.
!       (dkh, 03/28/05)
!  (4 ) Change name from ICS to SF, replace CMN_ADJ (dkh, ks, mak, cs
!  06/08/09)
!  (5 ) Now support strat fluxes LADJ_STRAT (hml, dkh, 02/20/12,
!  adj32_025) 
!******************************************************************************
!
      ! References to F90 modules
...
!      CALL EXPAND_NAME( FILENAME, N_CALC )
      ...
      ! Echo some input to the screen
      WRITE( 6, '(a)'   ) REPEAT( '=', 79 )
      WRITE( 6, '(a,/)' ) 'Y W  S F   F I L E   I N P U T'
      WRITE( 6, 100 ) TRIM( FILENAME )
 100  FORMAT( 'YW_READ_SF_FILE: Reading ', a )
     ...
     &           ( ( ( TRACER(I,J,L), I=1,NI ), J=1,NJ ), L=1,NL )
            ...
            ! Only process observation data (i.e. aerosol and
            ! precursors)
            IF ( CATEGORY(1:8) == 'IJ-EMS-$' ) THEN
...
!$OMP END PARALLEL DO
      ...
      !### Debug
      IF ( LPRT ) CALL DEBUG_MSG( '### YW_READ_SF_FILE: read file' )

      ! Return to calling program
      END SUBROUTINE YW_READ_SF_FILE
